...
description: "Periodic pass that keeps a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, at most one PR of proven corrections."
...
Only edit the verification skill's own directory under `.agents/skills/` (its SKILL.md, features/, and any harness scripts it owns), plus its `.claude/skills/` symlink. Never edit product code during a run: a behavior the map describes that the app no longer does is either doc drift (fix the map) or a product regression (report it, don't paper over it in docs).

## Pass

0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.agents/skills/verify-*/`, symlinked from `.claude/skills/`). Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target. Edit the `.agents/` copy, never the link; a `.agents/` skill with no `.claude/skills/` link is drift — relink it.
