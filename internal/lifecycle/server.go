...
func Serve(ctx context.Context, opts Options, emit func(Event)) error {
	...
	if opts.MySQLAddress != "" && opts.MySQLEnabled {
		mysqlServer, err = mysql.NewWithConfig(opts.MySQLAddress, mysql.Config{Catalog: store, Username: metadata.AdminAccount, PasswordHash: metadata.PasswordHash, TLSCertFile: opts.TLSCertFile, TLSKeyFile: opts.TLSKeyFile, MaxPreparedStmtCount: opts.MaxPreparedStmtCount, MaxAllowedPacket: opts.MaxAllowedPacket})
		if err != nil {
