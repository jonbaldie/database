# v0.1 query explanation schema prototype

> Throwaway planning artifact. This is a product-contract prototype, not database implementation code.

This prototype makes the accepted v0.1 query-explanation vocabulary concrete. The normative machine form is a versioned JSON physical-operator tree. The tabular form is a stable MySQL-oriented projection of the same tree.
...
- `statement`: statement text and public planning context
...
Submitted SQL is included. Prepared parameters expose only their ordinal position and SQL type; bound values are never echoed. Literal values already present in submitted SQL remain visible.
...
Consumers must ignore unknown object fields. Adding optional fields, warning or opportunity codes, or strategies is compatible within format version `1`. Removing fields, changing field meanings or types, or adding operator kinds requires a new integer format version. Existing strategy, warning, and opportunity identifiers cannot silently change meaning.
