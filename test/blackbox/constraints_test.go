...
func TestConstraintSurfaceThroughMySQL(t *testing.T) {
	...
	if result := client.query("TRUNCATE TABLE parent"); result.errCode != 1701 || !strings.HasPrefix(result.err, "42000") {
		t.Fatalf("truncate referenced parent failure: %#v", result)
	}
