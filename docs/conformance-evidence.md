...
This document maps every normative v0.1 promise in [Issue #1] to public
evidence. The principal system seam is the built `database` executable. The
black-box tests start that executable and use only supported operator commands,
the MySQL wire protocol and SQL, query explanations, and diagnostics HTTP
endpoints.

The map records evidence, not a claim of release acceptance. The performance
acceptance gate is still open. See the final status section.
...
executable, and the full production `messgo` gate. Pull requests also run the
changed-code `mutago` gate.

Use these public checks for the main product surfaces:

```sh
go test ./test/blackbox -count=1
go test ./cmd/database -count=1
...
## Application developer promises
...
| Issue #1 stories | Normative contract | Public evidence |
| --- | --- | --- |
| 52–56: explicit initialization, command family, results, exit classes, diagnostics, and secret input | [operator automation](operator-automation.md) | `TestInitializeCreatesStoppedInspectableInstance`, `TestInitializeAcceptsStdinAndRejectsInlinePassword`, `TestInitializeRejectsAmbiguousOrMalformedSecretInputs`, `TestCommandFailureIsObservable`, `TestServeEmitsTerminalOperatorResult`, `TestOperationReporterCorrelatesJSONProgressAndResult`, and `TestOperatorExitCodeClassesAreStable` |
| 57–59: configuration source precedence, invalid input rejection, and bounded limits | [server configuration](server-configuration.md) | `TestResolveConfigurationPrecedence`, `TestResolveConfigurationRejectsInvalidSources`, `TestResolveConfigurationCarriesEveryResourceLimitToServerBoundary`, `TestConfigurationRequiresDataDirectoryAndUsesRegistryBounds`, `TestMySQLSessionCeilingRejectsAdditionalConnections`, and `TestMySQLPreparedStatementCountIsBounded` |
| 60–62: online backup, empty-target restore, and forward-only upgrade | [operator automation](operator-automation.md) | `TestBackupRestoreWorkflowCreatesAndRestoresCompleteArtifactWithTerminalConfirmation`, `TestRestoreRejectsNonEmptyDestination`, `TestBackupInspectRejectsInvalidArtifact`, `TestRestoreRejectsUnsafeArtifact`, `TestUpgradeWorkflowRequiresMatchingBackupAndCompletesForwardOnly`, and `TestUpgradeResumesOnlyTheMarkedTarget` |
| 63–64: offline validation and corruption fail-closed behavior | [operator automation](operator-automation.md) | `TestDataValidateReportsCompleteHealthyState`, `TestDataValidateFailsClosedWithoutRepairingCorruption`, `TestDataInspectIsLimitedAndDoesNotRepairRecoveryArtifacts`, `TestServeRejectsDamagedInitializedDirectory`, `TestServeRejectsStructurallyDamagedCatalog`, and `TestServeRejectsUpgradeIncompleteDirectory` |
| 65–66: liveness, readiness, metrics, and sensitivity-safe lifecycle events | [server configuration](server-configuration.md) | `TestDiagnosticsHTTPContractIsObservableEndToEnd`, `TestDiagnosticsReadinessTracksStartupAndShutdown`, `TestDiagnosticsMethodsAndMetricsAreBounded`, and `TestLifecycleEventsHaveStableCodesAndOperationIdentity` |

## Release and contributor promises
...
| 69: every normative promise maps to public evidence | This document and [conformance guidance](conformance.md) | The map in this document. It becomes passing release evidence only after Issue #71 is accepted. |
...
This map has no unmapped Issue #1 story. It records the open performance
...
Until that evidence is accepted, v0.1 is not release-accepted and Issue #72
must remain open.
